...
export default function IssueTrackerTab() {
  ...
  const handleAutoGenerateDoD = () => {
    if (!formData.description) return;
    
    // Parse description for lines starting with -, *, or numbers
    ...
    if (newTasks.length > 0) {
      setFormData(prev => {
        ...
        const uniqueNewTasks = newTasks.filter(nt => !existingTasks.some(et => et.task === nt));
        return {
          ...prev,
          dod_items: [...existingTasks, ...uniqueNewTasks.map(t => ({ task: t, completed: false }))]
        };
      });
    } else {
      alert("No bullet points found in description (starting with '-', '*', or '1.')");
    }
  };
